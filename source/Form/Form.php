...
class Form
{
     ...
     * @var array
     */
    protected static $extraAttributes = ['prepend', 'append', 'label', 'description'];
     ...
     * @return mixed
     */
    public function label($id, $value, array $attributes = [])
    {
     ...
     * @return string
     */
    public function input($type, $id, $value = null, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function render($id, $tag, &$attributes, $content = null)
    {
        ...
        $field = Tag::create($tag, $attributes, $content);

        if ($label) {
            //$label = $this->label($id, $label, ['for' => $attributes['id']]);
        ...
        if ($description) {
            $field .= Tag::p(['class' => 'description', 'id' => $attributes['id'] . '-description'], $description);
        }
     ...
     * @return string
     */
    public function text($id, $value = null, array $attributes = [])
    {
     ...
     * @return string
     */
    public function password($id, $value = null, array $attributes = [])
    {
        return $this->input('text', $id, $value, $attributes);
    }
     ...
     * @return string
     */
    public function hidden($id, $value = null, array $attributes = [])
    {
     ...
     * @return string
     */
    public function special($id, $value)
    {
     ...
     * @return string
     */
    public function email($id, $value = null, array $attributes = [])
    {
     ...
     * @return string
     */
    public function url($id, $value = null, array $attributes = [])
    {
     ...
     * @return string
     */
    public function file($id, $value = null, array $attributes = [])
    {
     ...
     * @return string
     */
    public function number($id, $value = null, array $attributes = [])
    {
     ...
     * @return string
     */
    public function textarea($id, $value = null, array $attributes = [])
    {
        if (isset($attributes['size'])) {
            $size       = explode('x', $attributes['size']);
            $attributes = array_merge($attributes, ['cols' => (int)$size[0], 'rows' => (int)$size[1]]);
     ...
     * @return string
     */
    public function checkbox($id, $value = 1, $checked = false, array $attributes = [])
    {
        if ($value === null) {
            $value = true;
        }
     ...
     * @return string
     */
    public function radio($id, $value = null, $checked = false, array $attributes = [])
    {
        if ($value === null) {
            $value = $id;
        }
     ...
     * @return string
     */
    protected function checkable($type, $id, $value, $checked, $attributes)
    {
        $checked = $this->getCheckedState($type, $id, $value, $checked);
     ...
     * @return bool
     */
    protected function getCheckedState($type, $id, $value, $checked)
    {
        switch ($type) {
            case 'checkbox':
                return $this->getCheckboxState($id, $value, $checked);

            case 'radio':
                return $this->getRadioState($id, $value, $checked);
     ...
     * @return bool
     */
    protected function getCheckboxState($id, $value, $checked)
    {
     ...
     * @return bool
     */
    protected function getRadioState($id, $value, $checked)
    {
        if (($this->getSessionValue($id) === null) && ($this->getModelValue($id) === null)) {
            return $checked;
        }
     ...
     * @return string
     */
    public function select($id, array $values = [], $selected = null, array $attributes = [])
    {
        if (empty($values)) {
    ...
    protected function optionGroup($list, $label, $selected)
    {
        ...
        foreach ($list as $value => $display) {
            $options[] = $this->option($display, $value, $selected);
        }

        return Tag::optgroup(['label' => $label], $options);
    }
    ...
    /**
     * Create a choices field. It can be a select, a multiple select, a list of radio buttons or
     ...
     * @return string
     */
    public function choices($id, $values = [], $selected = null, array $attributes = [])
    {
        if (empty($values)) {
            $values = $this->getValueAttribute($id, $values);
        } elseif (is_callable($values)) {
        ...
        if ($expanded) {
            ...
            if ($description) {
                ...
                $field .= Tag::p(['class' => 'description', 'id' => $id . '-description'], $description);
            }
     ...
     * @return array
     */
    protected function getChoices($type, $id, $values, $selected, $attributes)
    {
        $choices = [];

        foreach ($values as $value => $label) {
            $checked   = is_array($selected) && in_array($value, $selected, false);
            $choices[] = [Tag::li(Tag::label([
     ...
     * @return string
     */
    public function image($id = null, array $attributes = [])
    {
     ...
     * @return string
     */
    public function token($referer = true)
    {
    ...
    protected function getAction(array $options)
    {
        ...
        $this->action = isset($options['action']) ? $options['action'] : $this->id();
     ...
     * @return mixed
     */
    protected function getValueAttribute($id, $value = null)
    {
        if ($id === null) {
            return $value;
        }
     ...
     * @return bool
     */
    protected function isSessionEmpty()
    {
