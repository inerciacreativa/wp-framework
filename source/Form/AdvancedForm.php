...
class AdvancedForm extends Form
{
	 ...
	 * @return string
	 */
	public function image_sizes(string $id, $selected = null, array $attributes = []): string
	{
	 ...
	 * @return string
	 */
	public function post_types(string $id, $selected = null, array $attributes = []): string
	{
	 ...
	 * @return string
	 */
	public function taxonomies(string $id, $selected = null, array $attributes = []): string
	{
		$attributes['multiple'] = Arr::get($attributes, 'multiple', true);
		$attributes['expanded'] = Arr::get($attributes, 'expanded', true);
	 ...
	 * @return string
	 */
	public function terms(string $id, $selected = null, array $attributes = []): string
	{
	 ...
	 * @return array
	 */
	protected function getObjectList(callable $callback, array &$attributes): array
	{
	...
	protected function getImageSizes(): array
	{
		...
		foreach ($sizes as $name) {
			if (isset($custom[$name])) {
				$dimension = [
					'width'  => $custom[$name]['width'],
					'height' => $custom[$name]['height'],
				];
			} else {
				$dimension = [
