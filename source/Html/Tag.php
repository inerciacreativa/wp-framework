...
class Tag implements \ArrayAccess
{
    ...
    public function content($content = null, $overwrite = false)
    {
        ...
        if (!\in_array($this->tag, static::$voidTags, false)) {
     ...
     * @return string
     */
    public function open($print = false): string
    {
     ...
     * @return string
     */
    public function close($print = false): string
    {
        if (\in_array($this->tag, static::$voidTags, false)) {
            $tag = '';
        } else {
    ...
    protected static function getAttributes(array $attributes): string
    {
        $attributes = Arr::map($attributes, function ($name, $value) {
            if (\in_array($name, static::$boolAttributes, false)) {
                return $value ? $name : '';
            }
            ...
            $value = \in_array($name, static::$urlAttributes, false) ? esc_url($value) : esc_attr($value);

            return sprintf('%s="%s"', $name, $value);
        });

        $result = trim(implode(' ', $attributes));
