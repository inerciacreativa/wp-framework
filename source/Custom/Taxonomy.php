...
use ic\Framework\Custom\MetaBox\TaxonomyMetaBox;
 ...
 * @property callable          $update_count_callback A function that will be called when the count of an associated
 *           object type is updated.
 *
 ...
 * @property bool              $show_admin_column     Whether to allow automatic creation of taxonomy columns on
...
class Taxonomy extends Type
{
    ...
    public function labels(array $labels = [])
    {
        ...
        if ($this->properties['hierarchical']) {
            Arr::forget($labels, ['popular_items', 'separate_items_with_commas', 'add_or_remove_items', 'choose_from_most_used']);
        } else {
    ...
    protected function addPostsFilter($type)
    {
        ...
        $label = Tag::label(
            ['for' => 'filter-by-' . $this->name, 'class' => 'screen-reader-text'],
            $this->object->labels->filter_items
        );

        $select = Tag::select(
            ['id' => 'filter-by-' . $this->name, 'name' => $this->name],
            Tag::option(['value' => ''], $this->object->labels->all_items)
        );

        foreach ($terms as $term) {
            $select->content(Tag::option(
                ['value' => $term->slug, 'selected' => $current === $term->slug],
