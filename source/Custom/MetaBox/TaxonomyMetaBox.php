...
class TaxonomyMetaBox
{
     ...
     */
    public function __construct(Taxonomy $taxonomy, $popular = true, $multiple = true, $allowNone = true)
    {
        ...
        Framework::getInstance()
                 ->addBackScript('taxonomy-meta-box.js', ['depends' => ['wp-lists'], 'hooks' => ['post.php', 'post-new.php']]);
     ...
     * @return array
     */
    protected function getPostTerms(\WP_Post $post)
    {
     ...
     * @return array
     */
    protected function getPopularTerms()
    {
    ...
    protected function getTabs()
    {
        ...
        $tabs = Tag::ul(['id' => "$taxonomy-tabs", 'class' => 'category-tabs'], [
     ...
     * @return Tag|null
     */
    protected function getPopularTab(array $selected, array $popular)
    {
        if (!$this->showPopular()) {
            return null;
        }
     ...
     * @return Tag
     */
    protected function getAllTab(\WP_Post $post, array $selected, array $popular)
    {
        $taxonomy = $this->taxonomy->name;
    ...
    protected function getNewTerm()
    {
        ...
        return Tag::div(['id' => "$taxonomy-adder", 'class' => 'wp-hidden-children'], [
     ...
     * @return array|null
     */
    protected function getNewParent()
    {
     ...
     * @return array
     */
    public function addNoneTerm(array $terms)
    {
        if ($this->allowNone()) {
            $none = (object)[
                'term_id' => 0,
     ...
     * @return Tag
     */
    public function getInput($term, array $attributes = [])
    {
        $taxonomy = $this->taxonomy->name;
        ...
        return Tag::li(['id' => $id, 'class' => $class],
            Tag::label(['class' => 'selectit'], [
                Tag::input(array_merge([
                    'type'       => $type,
                    'id'         => "in-$id",
                    'name'       => $name,
                    'value'      => $term->term_id,
                    'data-value' => $term->name, // Used for non hierarchical taxonomies.
                ], $attributes)),
                $space,
