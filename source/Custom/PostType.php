 ...
 * @property bool         $exclude_from_search   Whether to exclude posts with this post type from front end
 *           search
 *           results.
 ...
 * @property callable     $register_meta_box_cb  A callback that will be called when setting up the meta boxes for
 *           the edit form.
 *
 * @property string|array $capability_type       The string to use to build the read, edit, and delete
 *           capabilities.
 ...
 * @property string       $rest_controller_class An optional custom controller to use instead of
...
class PostType extends Type
{
     ...
     * @return static
     */
    public static function create($name, array $taxonomies = [])
    {
     ...
     * @param array  $taxonomies
     */
    public function __construct($name, array $taxonomies = [])
    {
        $this->defaults();
     ...
     * @return $this
     */
    public function menu($icon, $position = 20)
    {
        $this->menu_icon     = $icon;
     ...
     * @return $this
     */
    public function rewrite($slug, $front = true, $endpoint = EP_PERMALINK)
    {
        $this->rewrite = [
     ...
     * @return $this
     */
    public function filter_link($filter = true)
    {
        $this->filter = false;
        ...
        if (is_callable($filter)) {
            ...
            $this->filter = function ($link, \WP_Post $post) {
                ...
                $date = ['%year%' => $date[0], '%monthnum%' => $date[1], '%day%' => $date[2]];
     ...
     * @return array
     */
    protected function getMessages(\WP_Post $post)
    {
        ...
        if ($postType->publicly_queryable) {
            ...
            $preview   = ' ' . Tag::a(['href' => add_query_arg('preview', 'true', $permalink), 'target' => 'blank'], sprintf(__('Preview %s', 'ic-framework'), strtolower($this->singular)));
